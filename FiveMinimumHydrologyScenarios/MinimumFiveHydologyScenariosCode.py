...
for ensemble_input in sheet_names:
    ...
    for y in ensemble.columns[1:]: # Iterates through each trace
        ...
        for x in range(0, len(TraceY) - 4): # Iterates through each trace stopping 4 rows from the bottom
        ...
        if trace_min_sum < overall_min: # Finds the overall minimum of the ensemble and position of it
            ...
            fifth = TraceY[trace_min_pos + 4]



    # Adds values into variables
    if overall_trace:
        average = round((first + second + third + fourth + fifth) / 5, 1)
        all_results.append({
...
output_path = '/Users/anabelle/Documents/GitHub/ImmersiveModelLakeMead/MinimumFiveHydrologyResults.xlsx'
