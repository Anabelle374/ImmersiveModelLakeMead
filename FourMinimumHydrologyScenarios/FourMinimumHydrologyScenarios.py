...
#   four consecutive values of each ensemble.
...
for ensemble_input in sheet_names:
    ...
    for y in ensemble.columns[1:]: # Iterates through each trace
        ...
        for x in range(0, len(TraceY) - 3): # Iterates through each trace stopping 3 rows from the bottom
            result = TraceY[x] + TraceY[x + 1] + TraceY[x + 2] + TraceY[x+3]
        ...
        if trace_min_sum < overall_min: # Finds the overall minimum of the ensemble and position of it
            ...
            fourth = TraceY[trace_min_pos + 3]

    # Adds values into variables to put into table
    if overall_trace:
        average = round((first + second + third + fourth) / 4, 1)
        all_results.append({
...
output_path = '/Users/anabelle/Documents/GitHub/ImmersiveModelLakeMead/MinimumFourHydrologyResults.xlsx'
