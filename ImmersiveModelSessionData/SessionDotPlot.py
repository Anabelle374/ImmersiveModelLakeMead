...
for i in range(len(protect_elevs)):
    ...
    x_values = [protect_elevs[i]] * len(y_values)
    all_x.extend(x_values)
    all_y.extend(y_values)

# Separate red and blue points
red_x  = [x for x, y in zip(all_x, all_y) if y < 1.0]
red_y  = [y for x, y in zip(all_x, all_y) if y < 1.0]
blue_x = [x for x, y in zip(all_x, all_y) if y >= 1.0]
blue_y = [y for x, y in zip(all_x, all_y) if y >= 1.0]
...
plt.savefig("SessionDotPlot.png", dpi=200)
plt.show()
